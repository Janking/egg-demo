...
function Layer(config, selector) {
  ...
  // 创建遮罩层
  this.$backdrop = $('<div class="la-layer-backdrop"></div>')
  this.init()
}
...
Layer.prototype.init = function () {
  ...
  var $selector = _this.$selector = _this.$selector.length ? _this.$selector : $(template.replace('{{layerName}}', _this.$selector.selector.replace('#', '')))
...
Layer.prototype.showLayer = function (cutto) {
  ...
  utils.animateEndShim(_this.$content, function () {
    _this.$content.removeClass('la-layer-opening')
    // 触发show事件
  ...
  // 绑定 esc 键盘控制
  $(document).on('keyup.laui-layer', function (event) {
    if (event.keyCode === 27) {
