...
exports.default = {
  ...
  scrollBarWidth: function () {
    var scrollbarWidth;
    var $scrollDiv = (0, _jquery2.default)('<div/>');
    $scrollDiv.css({
      'width': 100,
      'height': 100,
      'overflow': 'scroll',
    ...
    (0, _jquery2.default)('html').append($scrollDiv);
...
var h337 = __webpack_require__(488);
var LAUI = {};
// 注入到jQuery原型对象
$.each([__webpack_require__(489), __webpack_require__(490), __webpack_require__(491)], function (index, component) {
  if ((typeof component === 'undefined' ? 'undefined' : _typeof(component)) === 'object' && !LAUI[component]) {
    $.extend(LAUI, component.default);
  }
});

// 注入到jQuery全局对象
$.each([__webpack_require__(492), __webpack_require__(493), __webpack_require__(494), __webpack_require__(495), __webpack_require__(496), __webpack_require__(497)
...
function gmdate(value) {
  var hours = Math.floor(value / 3600) > 9 ? Math.floor(value / 3600) : '0' + Math.floor(value / 3600);
  value %= 3600;
  var minutes = Math.floor(value / 60) > 9 ? Math.floor(value / 60) : '0' + Math.floor(value / 60);
  var seconds = value % 60 > 9 ? value % 60 : '0' + value % 60;
...
var $toolbarForm = $('.la-heatmap-form');
var DATA = null;
...
function updateInfo(resources) {
  var info = '<thead><tr><th>\u5C4F\u6570</th><th>PV</th><th>UV</th><th>IP</th><th>\u9000\u51FA\u6B21\u6570</th><th>\u9000\u51FA\u7387</th><th>\u5E73\u5747\u505C\u7559\u65F6\u957F</th></tr></thead>';
  ...
  var amouont = '<table class="c-table c-table-hover"><thead><tr><th>\u603B\u70B9\u51FB\u6B21\u6570</th><th>\u5143\u7D20\u6700\u5927\u70B9\u51FB\u6B21\u6570</th><th>PV</th></tr></thead><tbody><tr><td>' + (resources.clicks.allClickTimes || 0) + '</td><td>' + (resources.clicks.maxClickTimes || 0) + '</td><td>' + (resources.clicks.pv || 0) + '</td></tr></tbody></table>';

  var $infoLayer = $('#layer-info').LAUI('layer', {
...
var $datepicker = $('.la-heatmap-date').datepicker({
  toggleSelected: false,
  range: true,
  dateFormat: 'yyyy-mm-dd',
  multipleDatesSeparator: ' to ',
  onSelect: function onSelect(value, dateArr) {
    if (dateArr.length === 2) {
...
var $lineL = $('<div class="la-event-backdrop-left">').appendTo('body');
var $lineR = $('<div class="la-event-backdrop-right">').appendTo('body');
var $lineT = $('<div class="la-event-backdrop-top">').appendTo('body');
var $lineB = $('<div class="la-event-backdrop-bottom">').appendTo('body');
var clickEvent = {
  mouseenter: function mouseenter(event) {
    if (!/^(la-|^heatmap-|^datepicker-)/.test(event.target.className)) {
      ...
      var index = $(event.target.nodeName).index(event.target);
      var data = clickDataMap[event.target.nodeName] && clickDataMap[event.target.nodeName][index] ? clickDataMap[event.target.nodeName][index] : {};
      $.alert({
...
function heatmapDraw(data) {
  ...
  var demoWrapper = document.querySelector('.heatmap-canvas');
  var tooltip = document.querySelector('.la-heatmap-tooltip');
...
function readDepthDraw(data) {
  ...
  for (var i = 0, len = Math.ceil(height / 500); i < len; i++) {
    var step = (i + 1) * 500;
    var top = void 0;
    var isLast = false;
    if (step < height) {
  ...
  for (var _i = 0; _i < data.length; _i++) {
    $('.la-page-line-text').eq(data[_i].readDepth - 1).attr('data-tooltip', '\u9000\u51FA\u6B21\u6570\uFF1A' + data[_i].leavePv + '<br>\u9000\u51FA\u7387\uFF1A' + (data[_i].leavePv / data[_i].pv * 100).toFixed(2) + '%<br>PV\uFF1A' + data[_i].pv + '<br>UV\uFF1A' + data[_i].uv + '<br>IP\uFF1A' + data[_i].pv + '<br>\u5E73\u5747\u505C\u7559\uFF1A' + gmdate(parseInt(data[_i].duration / data[_i].pv)));
  }
...
var jQuery = __webpack_require__(103)
// 本插件仅供热力图使用……
// 2017-11-30 By Jankingzeng
  ...
  ajaxForm: function ajaxForm(options) {
    return this.each(function () {
      ...
      $selector.on('submit', function (event) {
        ...
        _jquery2.default.ajax(args).then(function (res) {
...
var GLOB_STRATEGY = {
  isNonEmpty: function isNonEmpty() {
    ...
    if (_jquery2.default.trim(value).length === 0) {
      return false;
    }
  },
...
Validate.prototype.init = function () {
  ...
  _jquery2.default.each(_this.cache, function (name, fields) {
    ...
    if (contextClassName) {
      ...
      fields.self.data('validateStatus', _jquery2.default.inArray(initStatus, statusArr));
    }
  });
};
...
Validate.prototype.mapping = function (options) {
  ...
  _jquery2.default.extend(true, this.cache, function () {
    var item = {};
    ...
    _jquery2.default.extend(true, item[options.required].matches, options.matches);
    return item;
  }());
};
...
Validate.prototype.remove = function (target) {
  ...
  for (name in cache) {
    ...
    if (_jquery2.default.inArray(requiredName, queue) !== -1) {
      if (_jquery2.default.inArray(type, ['checkbox', 'file', 'radio']) !== -1) {
      ...
      delete cache[name];
      var removeIndex;
      _jquery2.default.each(_this.options.collections, function (index, value) {
...
Validate.prototype.add = function (options) {
  ...
  if (options) {
    _jquery2.default.merge(_this.options.collections, options);
  }
...
Validate.prototype.bindEvent = function () {
  ...
  _jquery2.default.each(handleArr, function (key, value) {
    ...
    if (_jquery2.default.inArray(type, changeHandleArr) !== -1) {
...
Validate.prototype.verify = function (glob, eventName) {
  ...
  /**
       ...
       */
  _jquery2.default.each(matches, function (name, params) {
...
Validate.prototype.message = function (status, cache, matchesName) {
  ...
  $msgEl = this.options.globalMessage ? (0, _jquery2.default)(this.options.globalMessage) : cache.context;
  ...
  $target = (0, _jquery2.default)('<div class="validate-message ' + className + '" >' + msg + '</div>');
  $msgEl.append($target);
};
...
Validate.prototype.batch = function (circulation, requiredArr) {
  var _this = this;
  var status = [];
  _jquery2.default.each(this.cache, function (name, target) {
    var result, partValidResult;

    if (requiredArr && Object.prototype.toString.call(requiredArr) === '[object Array]') {
      _jquery2.default.each(requiredArr, function (index, value) {
        if (name === value) {
  ...
  return _jquery2.default.inArray(2, status) === -1;
};
...
Validate.prototype.changeStatus = function (target, code, text) {
  ...
  var stubData = _jquery2.default.extend(true, {}, _this.cache[target], { matches: { customer: { errMsg: text || '' } } });
  if ($target.length === 0) {
...
var scrollBarWidth = _utils2.default.scrollBarWidth;
...
function Layer(config, selector) {
  this.$selector = selector;
  this.config = _jquery2.default.extend(true, {}, defaults, config);
  // 创建遮罩层
  this.$backdrop = (0, _jquery2.default)('<div class="la-layer-backdrop"></div>');
  this.init();
}
...
Layer.prototype.init = function () {
  ...
  var template = '<div class="la-layer-box hide" id="{{layerName}}" tabindex="1"><div class="la-layer-content">' + config.content + '</div></div>';
  var $selector = _this.$selector = _this.$selector.length ? _this.$selector : (0, _jquery2.default)(template.replace('{{layerName}}', _this.$selector.selector.replace('#', '')));
  var $content = this.$content = $selector.find('.la-layer-content');
...
Layer.prototype.ajaxLoad = function () {
  ...
  _jquery2.default.loading(true, true);
...
Layer.prototype.bindEvent = function () {
  ...
  var $selector = _this.$selector;
  // 确认事件
  $selector.on(config.trigger + '.laui-layer', config.confirmHandle, function (event) {
    event.preventDefault();
  ...
  // 阴影层事件
  $selector.on(config.trigger + '.laui-layer', function (event) {
    if ((0, _jquery2.default)(event.target).is($selector)) {
      if (!config.shadow) {
        return false;
      }
      if ($body.find('.la-layer-loading').length) {
        return false;
      }
      _this.hideLayer();
  ...
  // 绑定关闭事件
  $selector.on(config.trigger + '.laui-layer', config.closeHandle, function (event) {
    _this.hideLayer();
    config.cancelCall.apply($selector, [event, _this]);
    return false;
  });
...
Layer.prototype.unBindEvent = function () {
  var _this = this;
  var config = _this.config;
  var $selector = _this.$selector;
  $selector.off(config.trigger + '.laui-layer', config.confirmHandle);
  $selector.off(config.trigger + '.laui-layer');
  $selector.off(config.trigger + '.laui-layer', config.closeHandle);
};
...
Layer.prototype.showLayer = function (cutto) {
  ...
  var isCutto = cutto;
  var Q = _jquery2.default.Deferred();
  ...
  _this.$content.off(_utils2.default.animateEnd);

  if (isCutto) {
    _this.$content.removeClass('la-layer-opening');
  } else {
    // 插入-遮罩-dom
  ...
  // 插入-弹层-css3显示动画
  _this.$content.addClass('la-layer-opening');

  _utils2.default.animateEndShim(_this.$content, function () {
    _this.$content.removeClass('la-layer-opening');
    // 触发show事件
  ...
  // 绑定 esc 键盘控制
  (0, _jquery2.default)(document).on('keyup.laui-layer', function (event) {
    if (event.keyCode === 27) {
      _this.$selector.trigger('click.laui-layer', config.closeHandle);
    }
  });
...
Layer.prototype.hideLayer = function (cutto) {
  ...
  // 插入-弹层-隐藏动画
  _this.$content.off(_utils2.default.animateEnd);
  _this.$content.addClass('la-layer-closing');
  if (!isCutto) {
    _this.$backdrop.removeAttr('style');
    _utils2.default.transitionEndShim(_this.$backdrop, function () {
      _this.$backdrop.remove();
    }, config.animateDisable);
  }
  _utils2.default.animateEndShim(_this.$content, function () {
    // 插入-遮罩-隐藏动画
    _this.$content.removeClass('la-layer-closing');
    // 隐藏弹层
  ...
  // 绑定 esc 键盘控制
  (0, _jquery2.default)(document).off('keyup.laui-layer');
  _this.unBindEvent();
  return Q;
};

Layer.prototype.cutTo = function (nextId, currentId) {
  if (!(0, _jquery2.default)(nextId).length) {
    console.warn('Can\'t find layer \'' + nextId + '\',the config.dynamic must set false');
    return false;
  }
  var nextLayer = (0, _jquery2.default)(nextId).data('layer');
  var currentLayer = (currentId ? (0, _jquery2.default)(currentId) : this.$selector).data('layer');
...
exports.default = {
  alert: function alert(options) {
    var param = _jquery2.default.extend({
      container: 'body',
      obj: '#la-message',
      text: '',
      timeout: 1000,
    ...
    // 初始化jQuery对象
    var obj = (0, _jquery2.default)(param.obj).length === 0 ? (0, _jquery2.default)('<div ' + callerStyle + '="' + param.obj.slice(1) + '" />').appendTo('body') : (0, _jquery2.default)(param.obj);
    // 判断状态
...
exports.default = {
  dialog: function dialog(options) {
    ...
    if (!_jquery2.default.dialogBackdrop) {
      _jquery2.default.dialogBackdrop = (0, _jquery2.default)('<div class="IUI-dialog-backdrop"></div>');
      $body.append(_jquery2.default.dialogBackdrop);
    }
    ...
    container.on('touchstart.iui-dialog click.iui-dialog', '.IUI-dialog-confirm', function () {
    ...
    container.on('touchstart.iui-dialog click.iui-dialog', '.IUI-dialog-cancel,.IUI-dialog-close', function () {
      if (config.cancel.call(this, deferred) === false) {
        return false;
      }
    ...
    if (config.keyboard) {
      (0, _jquery2.default)(document).off('keyup.iui-dialog').on('keyup.iui-dialog', function (event) {
        // keyCode => esc
        if (event.keyCode === 27) {
    ...
    function show(target) {
      var screenH = document.documentElement.clientHeight;
      var GtIE10 = document.body.style.msTouchAction === undefined;
      target.find('.IUI-dialog-main').off(_utils2.default.animateEnd);
      _jquery2.default.dialogBackdrop.off(_utils2.default.transitionEnd);
      // 当body高度大于可视高度，修正滚动条跳动
      // tmd,>=ie10的滚动条不需要做此修正
      if ((0, _jquery2.default)('body').height() > screenH & GtIE10) {
        $body.css({ 'border-right': scrollBarWidth + 'px transparent solid', 'overflow': 'hidden' });
      }

      target.removeClass('hide');
      _jquery2.default.dialogBackdrop.attr('style', 'opacity: 1;visibility: visible;');
      target.find('.IUI-dialog-main').addClass('dialog-opening');
      target.focus();
      _utils2.default.animateEndShim(target.find('.IUI-dialog-main'), function () {
    ...
    function hide(target) {
      (0, _jquery2.default)([$body, target]).off('touchstart.iui-dialog click.iui-dialog');
      target.addClass('dialog-closing');
      _jquery2.default.dialogBackdrop.removeAttr('style');
      _utils2.default.transitionEndShim(_jquery2.default.dialogBackdrop, function () {
        target.remove();
    ...
    function create() {
      ...
      var $container = (0, _jquery2.default)('<div class="IUI-dialog-container hide" tabindex="10"><div class="IUI-dialog-main ' + config.status + '">' + _main + '</div></div>');
...
function Loading(options, type) {
  ...
  // 是否全局模式
  var isGlobal = this instanceof _jquery2.default;
  // 作用域元素
  var $context = (0, _jquery2.default)('body');

  _jquery2.default.extend(defaults, isTempConfig ? options : {});
  ...
  var $loading = (0, _jquery2.default)(createTemplate(defaults, isGlobal));
...
_jquery2.default.fn.loading = Loading;

exports.default = { loading: Loading };
...
var positionCalc = {
  ...
  up: function up(options, template) {
    ...
    var destW = template.outerWidth();
    var destH = template.outerHeight();
    var left = srcW / 2 + srcX - destW / 2;
    return [left < 0 ? 0 : left, srcY - destH - _this.config.offsetY];
  },
...
function Tooltip(options) {
  var _this = this;
  _this.config = _jquery2.default.extend(defaults, options);
  _this.$container = (0, _jquery2.default)(_this.config.container);
  _this.init();
}
...
Tooltip.prototype = {
  init: function init() {
    ...
    _this.$container.on('mouseenter.iui-tooltip', handler, _jquery2.default.proxy(_this.show, _this));
    _this.$container.on('mouseleave.iui-tooltip', handler, _jquery2.default.proxy(_this.hide, _this));
    _this.$container.on('focus.iui-tooltip', handler, _jquery2.default.proxy(eventAction.focus, _this));
  ...
  show: function show(event) {
    ...
    _this.$template = (0, _jquery2.default)('<div id="tooltip-' + +new Date() + '" class="IUI-tooltip tooltip-' + size + ' tooltip-' + direct + '">' + text + '</div>');
